...
import { SecurityPolicyDecision, ProjectSecurityScan } from "../projects/project-security-scan.entity";
...
export class InfrastructureReadinessService {
  ...
  async getDeploymentReadiness(projectId: string, user: User) {
    ...
    checks.push(this.checkBoolean("github_actions", "GitHub Actions workflow", Boolean(this.config.get<string>("GITHUB_TOKEN") && this.config.get<string>("GITHUB_ACTIONS_WORKFLOW_FILE", "deploy.yml")), "GitHub Actions dispatch is configured.", "GitHub Actions workflow dispatch is not configured."));
  ...
  private costCheck(estimate: ProjectCostEstimate | null): DeploymentReadinessCheck {
    ...
    if ([CostEstimateStatus.NO_APPROVAL_REQUIRED, CostEstimateStatus.APPROVED].includes(estimate.status)) {
